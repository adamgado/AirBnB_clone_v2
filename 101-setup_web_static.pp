...
$nginx_conf = "server {
    ...
    location /hbnb_static {alias /data/web_static/current; index index.html index.htm;}
    location /redirect_me {return 301 http://cuberule.com/;}
...
file {'/data/web_static/releases/test/index.html': ensure  => 'present', content => "simple content\n"} ->
...
file {'/var/www/html/index.html': ensure  => 'present', content => "simple content\n"} ->
file {'/var/www/html/404.html': ensure  => 'present', content => "\n"} ->
file {'/etc/nginx/sites-available/default': ensure  => 'present', content => $nginx_conf} ->
exec {'nginx restart': path => '/etc/init.d/'}
